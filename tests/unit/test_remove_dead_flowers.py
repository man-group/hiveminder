...
def test_no_expiry_causes_error():
    game = GameState(game_params=DEFAULT_GAME_PARAMETERS,
                     ...
                     flowers=[[Flower(sentinel.x,
                                      sentinel.y, DEFAULT_GAME_PARAMETERS,
                                      sentinel.potency,
                                      sentinel.visits,
                                      expires=None)]],
                     turn_num=0,
...
def test_flowers_removed_if_expired():
    flowers = [
        Flower(sentinel.x,
               sentinel.y, DEFAULT_GAME_PARAMETERS,
               sentinel.potency,
    ...
    assert game.boards[0].flowers == (flowers[1],)


def test_keep_one_flower_if_all_expired():
    flowers = [
        Flower(sentinel.x,
               sentinel.y, DEFAULT_GAME_PARAMETERS,
               sentinel.potency,
...
def test_only_expired_flowers_removed_from_mixed_bunch():
    game = GameState(game_params=DEFAULT_GAME_PARAMETERS,
                     ...
                     flowers=[[Flower(sentinel.x1,
                                      sentinel.y1, DEFAULT_GAME_PARAMETERS,
                                      sentinel.potency1,
                                      sentinel.visits1,
                                      expires=10),
                               Flower(sentinel.x2,
                                      sentinel.y2, DEFAULT_GAME_PARAMETERS,
                                      sentinel.potency2,
                                      sentinel.visits2,
                                      expires=50),
                               Flower(sentinel.x3,
                                      sentinel.y3, DEFAULT_GAME_PARAMETERS,
                                      sentinel.potency3,
                                      sentinel.visits3,
                                      expires=100)]],
                     turn_num=75,
